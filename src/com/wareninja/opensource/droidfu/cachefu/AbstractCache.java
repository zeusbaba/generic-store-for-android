...
import com.wareninja.opensource.droidfu.support.StringSupport;
...
public abstract class AbstractCache<KeyT, ValT> implements Map<KeyT, ValT> {
    ...
    public boolean enableDiskCache(Context context, int storageDevice) {
        ...
        if (cacheDirName!=null && !cacheDirName.trim().equals(""))
        	rootDir += "/" + cacheDirName;
        else
        	rootDir += "/" + appContext.getPackageName();
        ...
        if (!isDiskCacheEnabled) {
            ...
            if (LOGGING.DEBUG)Log.d(name, "enabled write through to " + diskCacheDirectory);
        }
    ...
    private void cacheToDisk(KeyT key, ValT value) {
        ...
        try {
            ...
            BufferedOutputStream ostream = new BufferedOutputStream(new FileOutputStream(file), 10240);//YG: 10k=10240
    ...
    public synchronized ValT get(Object elementKey) {
        ...
        if (value != null) {
            // memory hit
            if (LOGGING.DEBUG)Log.d(name, "MEM cache hit for " + key.toString());
            return value;
        }
        ...
        if (file.exists()) {
            // disk hit
            if (LOGGING.DEBUG)Log.d(name, "DISK cache hit for " + key.toString());
            try {
